...
export default function JobListingForm({ onSubmit, initialJobListing = DEFAULT_VALUES }: JobListingFromProps) {
  const form = useForm<JobListingFormValues>({ resolver: zodResolver(jobListingFormSchema), defaultValues: initialJobListing });
  ...
  return (
    <>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mb-5">
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <JobSelectFormField control={form.control} name="type" label="Type" options={JOB_LISTING_TYPES as any} />
            ...
            <FormField control={form.control} name="salary" render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input type="number" {...field} onChange={(e) => field.onChange((e.target as HTMLInputElement).valueAsNumber)} value={isNaN(field.value as any) ? "" : (field.value as any)} />
                </FormControl>
          ...
          <div className="flex gap-2 justify-end">
            <Button type="button" onClick={() => setShowPreview((state) => !state)} variant="outline">
              {showPreview ? "Hide Preview" : "Show Preview"}
            </Button>
            <Button type="submit">{form.formState.isSubmitting ? <LoadingSpinner /> : "Save"}</Button>
          </div>
        </form>
      </Form>
      ...
      {showPreview && (
        <JobListingGrid>
          <JobListingCard job={jobListingValues as any} footerBtns={<div>Preview</div>} />
        </JobListingGrid>
      )}
    </>
  );
